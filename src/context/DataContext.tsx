...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  // Load data from localStorage or migrate from Supabase
  useEffect(() => {
    const migrateData = async () => {
      const isMigrated = localStorage.getItem('data_migrated_v2');
      if (isMigrated) return;

      console.log('Migrating data from Supabase to localStorage...');
